...
def get_timeseries(instance, stf, com, sites, timesteps=None):
    ...
    Usage:
        ...
        voltage_angle, voltage_magnitude = get_timeseries(instance, commodity, sites, timesteps)
    ...
    Returns:
        a tuple of (created, consumed, storage, imported, exported, voltage_angle, voltage_magnitude) with
        ...
        - voltage_angle: timeseries of voltage angles (DCPF mode)
        - voltage_magnitude: timeseries of voltage magnitudes (ACPF mode)
        Note: DSM (demand-side management) functionality has been removed
    """
    ...
    # DSM (Demand Side Management) functionality removed - no longer needed
    # DSM variables removed: dsmup, dsmdo, delta, shifted, dsm
    # Demand is no longer modified by DSM shifting

    # JOINS
    created = created.join(stock)  # show stock as created
    consumed = consumed.join(demand.rename('Demand'))  # use original demand instead of shifted
    ...
    return created, consumed, stored, imported, exported, voltage_angle, voltage_magnitude  # DSM removed from return
