...
def plot(prob, stf, com, sit, dt, timesteps, timesteps_plot,
    ...
    (created, consumed, stored, imported, exported,
     voltage_angle, voltage_magnitude) = get_timeseries(prob, stf, com, sit, timesteps)  # DSM removed
    ...
    # DSM plotting functionality removed - demand side management no longer available
    # DSM plotting disabled since DSM functionality was removed
    ...
    all_axes = []
    # DSM subplot removed - always use 2-row layout since DSM plotting was removed
    ...
    ax0.plot(hoursteps, demand.values / dt[0], linewidth=0.8,
             color=to_color('Unshifted'))
 # line plot for demand commodities (DSM mode removed - demand no longer shifted)
    ...
    try:
        ...
        sp1 = ax1.stackplot(hoursteps, stored.values, linewidth=0.15)
    # DSM conditional logic removed - always add time axis label since DSM plotting was removed
    ...
    # DSM plotting removed - demand side management functionality no longer available
    # The DSM subplot is no longer created since plot_dsm is always False
