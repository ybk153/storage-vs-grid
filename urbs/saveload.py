...
# removed def rename_duplicate_columns(df): uhp

# create_result_cache used for save()
        ...
        for prob_ in prob.values():
            for name in prob_._data.keys():
                if name not in ['trafo_node', 'mainbusbar_node']:
                    ...
                    elif name in ['demand', 'supim', 'eff_factor', 'availability']:  # UHP removed
        ...
        with pd.HDFStore(filename, mode='w') as store:
            for name in data_all.keys():
                try:
                    ...
                    pass
                if name not in ['trafo_node', 'mainbusbar_node']:
                    # UHP column processing removed - UHP functionality no longer needed
