...
def read_input(input_files, year):      # used in runfunction.py
    ...
    sup     = []
    bsp     = []
    # DSM list removed - demand side management functionality no longer needed
    ef      = []
    av      = []
    # UHP array removed - no longer needed
    for filename in input_files:
        with pd.ExcelFile(filename) as xls:

            global_prop = xls.parse('Global').set_index(['Property'])
            # create support timeframe index
            if ('Support timeframe' in  xls.parse('Global').set_index('Property').value):
            ...
            # Transmission, Storage (DSM removed)

            if 'Transmission' in xls.sheet_names:
                transmission = xls.parse('Transmission').set_index(['Site In', 'Site Out', 'Transmission', 'Commodity'])
            ...
            # DSM sheet parsing removed - demand side management functionality no longer needed
            ...
            # UHP parsing removed - this functionality is no longer needed
    ...
    try:
        ...
        # DSM concatenation removed - demand side management functionality no longer needed
        buy_sell_price      = pd.concat(bsp, sort=False)
        eff_factor          = pd.concat(ef, sort=False)
        availability        = pd.concat(av, sort=False)
        # UHP concatenation removed - no longer needed
    except KeyError:
        pass
    ...
    data = {
        ...
        'storage'           : storage,
        # DSM data removed - demand side management functionality no longer needed
        ...
        # UHP data removed - this functionality is no longer needed
    }
...
def pyomo_model_prep(data, timesteps):
    ...
    if m.mode['tra']:
        ...
        # create no expansion dataframes
        tra_const_cap = transmission[transmission['inst-cap'] == transmission['cap-up']]        # tra_const_cap: installed unexpandable transmission capacity
    ...
    # DSM mode and dictionary creation removed - demand side management functionality no longer needed
    if m.mode['bsp']:
    ...
    # UHP dictionary initialization removed - this functionality is no longer needed
    if m.mode['tdy']:
    ...
    # UHP building parameters removed - this functionality is no longer needed
    # Previously set up U, V, C values for thermal building model
    ...
    # cost_factor = discount factor * effective distance -> discount(present value of future costs/payments)
    # invcost_factor = discount & annuity -> invcost_factor = annuity factor for one year problem
    if m.mode['int']:
    ...
    # Add debug prints for capacity dictionaries
    print("\n=== DEBUG: Process Dictionary (m.process_dict) ===")
    print("Keys related to capacity:")
    capacity_keys = ['inst-cap', 'cap-up', 'cap-lo']
    for key in capacity_keys:
        if key in m.process_dict:
            print(f"  {key}: {dict(list(m.process_dict[key].items())[:5])}...")  # Show first 5 entries
    
    print("\n=== DEBUG: Constant Capacity Dictionary (m.pro_const_cap_dict) ===")
    print(f"Number of entries: {len(m.pro_const_cap_dict)}")
    print(f"Sample entries: {dict(list(m.pro_const_cap_dict.items())[:10])}")  # Show first 10 entries
    
    print("\n=== DEBUG: pro_const_cap DataFrame before conversion ===")
    print(f"Shape: {pro_const_cap.shape}")
    print("Sample entries:")
    print(pro_const_cap.head())
    
    # Check specific process if it exists
    sample_process_keys = list(m.process_dict['inst-cap'].keys())[:3]
    print(f"\n=== DEBUG: Sample process capacity comparison ===")
    for key in sample_process_keys:
        inst_cap = m.process_dict['inst-cap'].get(key, 'Not found')
        cap_up = m.process_dict['cap-up'].get(key, 'Not found')
        in_const_dict = key in m.pro_const_cap_dict
        print(f"Process {key}:")
        print(f"  inst-cap: {inst_cap}")
        print(f"  cap-up: {cap_up}")
        print(f"  In const_cap_dict: {in_const_dict}")
        print(f"  Equal capacities: {inst_cap == cap_up}")

    if m.mode['tra']:
        m.mode['exp']['tra'] = identify_expansion(
            tra_const_cap['inst-cap'],
            transmission['inst-cap'].dropna())
        m.tra_const_cap_dict = tra_const_cap['inst-cap'].to_dict()      # dict of installed unexpandable transmission capacities with values of installed capacity
...
# removed def get_cluster_data(data, cluster):
