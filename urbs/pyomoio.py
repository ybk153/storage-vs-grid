...
# Entity Data Extraction: get_entity() and get_entities() extract data from Pyomo model entities (Sets, Parameters, Variables, Constraints) and convert them to pandas Series or DataFrames with appropriate indexing.
# Model Introspection: list_entities() and _get_onset_names() provide tools to inspect model structures, retrieve domain information, and understand relationships between model components.
...
def list_entities(instance, entity_type):
    ...
    def filter_by_type(entity, entity_type):
        if entity_type == 'set':
            return isinstance(entity, pyomo.Set) #and not getattr(entity, 'virtual', False)
    ...
    # create entity iterator - Python 3 compatible
...
def _get_onset_names(entity):
    ...
    if isinstance(entity, pyomo.Set):
        ...
        if entity.dim() > 0:
            # Use index_set() method instead of deprecated _index attribute
            try:
                index_set = entity.index_set()
                if index_set is not None:
                    labels = _get_onset_names(index_set)
            except (AttributeError, TypeError):
                # Fallback for older PyOmo versions or special cases
                try:
                    if hasattr(entity, '_index') and entity._index:
