...
def run_tsam(data, noTypicalPeriods, hoursPerPeriod, cross_scenario_data, seasonality):
    # UHP parameter removed - this functionality is no longer needed
    ...
    # UHP condition removed - concatenate without UHP data
    ...
    # Set small weights for non-clustering commodities
    weights = {(sit,com): 10**-6 for (sit, com) in time_series_data.columns if ((com in ['electricity-reactive',
                                                                                     'electricity_import',
                                                                                     'electricity_feed_in']))}

    # LVDS: considered time-series for clustering are:
    #       electricity demand (col2 = 'electricity')
    ...
    # Calculate column numbers for normalizing weights
    electricity_column_number = len([col for col in time_series_data.columns if col[1] == 'electricity'])
    solar_column_number = len([col for col in time_series_data.columns if col[1].startswith('solar')])
    

    # Set weights based on column type - focus on electricity demand and solar supply for clustering
    for col in time_series_data.columns:
        if col[1] == 'electricity' and electricity_column_number > 0:
            weights[col] = 1 / electricity_column_number
        if col[1].startswith('solar') and solar_column_number > 0:
            weights[col] = 1 / solar_column_number


    ## LVDS: add the typical week that includes the hour having the lowest annual temperature
    # UHP functionality removed

    # If extreme events included:
    addMeanMin_cols = None
    addMeanMax_cols = None
    ...
    ###prepare datetime vector which is required by tsam method - adjusted for 35136 timesteps
    date_hour = np.arange(datetime(2016, 1, 1), datetime(2016, 1, 1)+timedelta(hours=35136), timedelta(hours=1)).astype(datetime).T
    ...
    print('Applying TSAM time series aggregation')

    if cross_scenario_data and 'predefClusterCenterIndices' in cross_scenario_data and 'predefClusterOrder' in cross_scenario_data:
        print('coordinated tsam step with saved clusters config in cross_scenario_data')
        aggregation = tsam.TimeSeriesAggregation(time_series_data, noTypicalPeriods=noTypicalPeriods, 
                                                predefClusterOrder=cross_scenario_data['predefClusterOrder'],
                                                predefClusterCenterIndices=cross_scenario_data['predefClusterCenterIndices'], 
                                                hoursPerPeriod=hoursPerPeriod,
                                                clusterMethod=clusterMethod, 
                                                weightDict=weights, 
                                                extremePeriodMethod=extremePeriodMethod, 
                                                addMeanMin=addMeanMin_cols,
                                                addMeanMax=addMeanMax_cols)
        ...
        # Save the cross-scenario data after aggregation for potential future use
    ...
    if seasonality == False:
        ...
        occurence_weight_factor[last_period_nr] = occurence_weight_factor[last_period_nr] - 1 \
                                                + (35136 % hoursPerPeriod)/hoursPerPeriod
        ...
        occurence_weight_factor[last_period_nr][-1] = occurence_weight_factor[last_period_nr][-1] \
                                                      - 1 + (35136 % hoursPerPeriod)/hoursPerPeriod #consider that 35136 timesteps normally can not be divided in an integer number of typeperiods
    ...
    # TransDist-related code removed
    ...
    # UHP code removed - no longer needed for simplified model
    ...
    # UHP rounding removed - no longer needed
...
def add_typeperiod(m, hoursPerPeriod):
    ...
    if m.mode['tsam'] and m.mode['tsam_season']:
        ...
        t_startofperiod_list = []
        for hour in t_endofperiod_list:
            start_hour = hour + 1 - (hoursPerPeriod * m.dt.value)
            # Ensure start hour is not negative and is within valid timestep range
            start_hour = max(start_hour, min(m.timesteps))
            t_startofperiod_list.append(start_hour)
        ...
        m.t_endofperiod = pyomo.Set(
            ...
            ordered=True,
            doc='timestep at the end of each timeperiod')
        # Storage cyclicity constraints always applied for full co-optimization
        if m.mode['sto']:
            m.res_storage_state_cyclicity_typeperiod = pyomo.Constraint(
                m.t_endofperiod, m.sto_tuples,
