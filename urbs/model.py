...
def create_model(data, dt=1, timesteps=None, objective='cost', hoursPerPeriod=None, weighting_order=None,
                 assumelowq=True, dual=True, bui_react_model=False, flexible_heat=True):
    ...
    m.weight = pyomo.Param(
        ...
        #initialize=float(35136) / (len(m.timesteps) - 1 * dt),
    ...
    '''
    Parameters list:
    m.weight:           Pre-factor for variable costs and emissions for an annual result
    m.dt:               Time step duration (in hours), default: 1
    m.obj:              Specification of minimized quantity, default: "cost"
    '''
    ...
    '''
    Set list:
    m.t:                            Set of timesteps                                                                            from run.py or data['demand'].index.tolist()
    m.tm:                           Set of modelled timesteps (i.e. excluding init time step for storage)                       from timesteps
    m.stf:                          Set of modeled support timeframes (e.g. years)
    m.sit:                          Set of sites (e.g. north, middle, south...)
    m.com:                          Set of commodities (e.g. solar, wind, coal...)
    m.com_type:                     Set of commodity types (i.e. SupIm, Demand, Stock, Env)
    m.pro:                          Set of conversion processes (e.g. Wind turbine, Gas plant, Photovoltaics...)
    m.cost_type:                    Set of cost types (hard-coded)
    m.sit_tuples:                   Combinations of support timeframes and sites
    m.sit_tuples_ac:                Combinations of support timeframes and sites with ac characteristics
    m.sit_slackbus:                 Set of all reference nodes in defined microgrids
    m.sit_power_price_tuples:       Combinations of support timeframes and sites with retail                            if m.mode['power_price'] is True
    # m.sit_temperature_tuples:     Combinations of support timeframes and sites with U, V, C values                    UHP mode removed
    m.com_tuples:                   Combinations of defined commodities, e.g. (2018,Mid,Elec,Demand)
    m.pro_tuples:                   Combinations of possible processes, e.g. (2018,North,Coal plant)
    m.com_stock:                    Commodities that can be purchased at some site(s)  
    
    m.operational_pro_tuples:       Operational status of technologies, e.g. (sit, pro, stf, stf_later)                 if m.mode['int'] is True
    m.inst_pro_tuples:              Combinations of installed processes, e.g. (sit, pro, stf)                           if m.mode['int'] is True
        
        Commodity
    m.com_supim:                    Commodities that have intermittent (timeseries) input
    m.com_demand:                   Commodities that have a demand (implies timeseries)
    m.com_env:                      Commodities that (might) have a maximum creation limit
    m.com_stock:                    Commodities that can be purchased at some site(s) 

        Process
    m.pro_area_tuples:              Removed Processes and Sites with area Restriction
    m.pro_cap_new_block_tuples:     Processes with new capacities built in blocks
    m.pro_inv_cost_fix_tuples:      Processes with fixed investment cost portions
    m.pro_input_tuples:             Commodities consumed by process by site,''e.g. (2020,Mid,PV,Solar)'
    m.pro_output_tuples:            Commodities produced by process by site, e.g. (2020,Mid,PV,Elec)
    m.pro_output_tuples_reactive:   Elec-Reactive produced by process by site, e.g. (2020, node1, PV_private, Elec-Reactive)
    m.pro_rampupgrad_tuples:        Processes with maximum ramp up gradient smaller than timestep length
    m.pro_rampdowngrad_tuples:      Processes with maximum ramp down gradient smaller than timestep length
    m.pro_decommissionable_tuples:  Processes which can be decommissioned
    ...
    m.sit_tuples = pyomo.Set(
        within=m.stf * m.sit,
        initialize=tuple(m.site_dict['base-voltage'].keys()),
    ...
    m.sit_tuples_ac = pyomo.Set(
        ...
        doc='Combinations of support timeframes and sites with ac characteristics/ min-voltage > 0')
    ...
    # UHP sit_temperature_tuples removed - used for thermal building model
    ...
    # process tuples for area rule -> removed
    ...
    print("DEBUG: m.com set:", list(m.com))
    print("DEBUG: m.pro set:", list(m.pro))
    print("DEBUG: r_in_dict keys sample:", list(m.r_in_dict.keys())[:5])
    print("DEBUG: Checking if 'solar_6' in m.com:", 'solar_6' in m.com)
    print("DEBUG: Checking if 'SGen_6' in m.pro:", 'SGen_6' in m.pro)
    print("DEBUG: Checking if 'Bus_1' in m.sit:", 'Bus_1' in m.sit)
    print("DEBUG: Checking if 2025 in m.stf:", 2025 in m.stf)
    ...
    # m.pro_maxgrad_tuple of urbs replaced by m.pro_rampupgrad_tuples and m.pro_rampdowngrad_tuples
    ...
    '''
        Cost
    m.costs:                Costs by type (EUR/a)

        Commodity
    m_e_co_stock:           Use of stock commodity source (MW) per timestep

        Site
    m.peak_injection:       Peak injection/feed-in in site                                  if mode['power_price'] is True
    m.abs_injection:        Absolute injection/feed-in in site                              if mode['power_price'] is True
    # m.temperature:        Temperature in building                                         UHP mode removed
    # m.temperature_slack:  Temperature slack in building                                   UHP mode removed
    
        Process
    m.cap_pro_new:          New process capacity (MW)
    m.cap_decommissioned:   Decommissioned process capacity (MW)
    m.cap_pro:              total process capacity                                          as pyomo.expression
    m.tau_pro:              Power flow (MW) through process 
    m.e_pro_in:             Power flow of commodity into process (MW) per timestep
    m.e_pro_out:            Power flow out of process (MW) per timestep
    m.pro_cap_unit:         Number of newly installed capacity units
    m.pro_cap_expands:      Boolean variable whether a process is expanded
    ...
    m.e_co_stock = pyomo.Var(
        ...
        #within=pyomo.Reals,  # Changed from pyomo.NonNegativeReals
    ...
    # UHP temperature variables removed - used for thermal building model
    ...
    if m.mode['sto']:
        m = add_storage(m)
    # DSM mode and add_dsm call removed - demand side management functionality no longer needed
    if m.mode['bsp']:
    ...
    if (m.mode['tve'] or m.mode['onoff'] or  m.mode['ava'] or m.mode['minfraction']):
        ...
        print("DEBUG: Creating empty sets for advanced processes")
        ...
        m.pro_availability_tuples = pyomo.Set(
            within=m.stf * m.sit * m.pro,
            doc='empty set for process availability - not used without advanced processes')
    '''
    Set list (if not using advanced processes: mode['tve'], mode['onoff'], mode['ava'], mode['minfraction']):

    m.pro_timevar_output_tuples:            empty set needed for (partial) process output
    m.pro_on_off_tuples:                    empty set needed for (partial) on/off processes   
    m.pro_on_off_input_tuples:              Commodities for on/off input
    m.pro_on_off_output_tuples:             Commodities for on/off output
    m.pro_partial_on_off_tuples:            Processes with partial input/output which can be turned off
    m.pro_partial_on_off_input_tuples:      Commodities with partial input ratio, e.g. (2020,Mid,Coal PP,Coal)
    m.pro_partial_on_off_output_tuples:     Commodities for on/off output with partial behaviour
    m.pro_start_up_tuples:                  Processes with fix start up costs
    m.pro_rampup_start_tuples:              Processes with different starting ramp up gradient
    m.pro_minfraction_tuples:               empty processes with constant efficiency and minimum working load which cannot be turned off
    m.pro_minfraction_output_tuples:        empty commodities with minimum working load and NO partial output ratio
    m.pro_partial_tuples:                   empty processes with partial input/output which cannot be turned off
    m.pro_partial_input_tuples:             empty commodities with partial input ratio
    m.pro_partial_output_tuples:            empty commodities with partial output ratio
    '''
    ...
    if m.mode['power_price']: #LVDS
        # Power price injection constraints always applied for full co-optimization
    ...
    # UHP temperature constraints removed - used for thermal building model
...
def res_vertex_rule(m, tm, stf, sit, com, com_type):
    ...
    # DSM surplus calculation removed - demand side management functionality no longer needed

    # UHP thermal building model functionality removed
    # Space heat demand is now handled as a standard demand commodity read directly from input data
...
def def_process_capacity_rule(m, stf, sit, pro):
    if m.mode['int']:  # operational mode of function
        ...
        if (stf, sit, pro) in m.pro_const_cap_dict:
            cap_pro = m.process_dict['inst-cap'][(stf, sit, pro)]
            print("no cap new")
        else:
...
# removed def res_area_rule(m, stf, sit):
...
# UHP temperature constraint rules removed - these functions were used for the thermal building model
# that calculated space heating demand endogenously. Now space heating is read directly from input data.
# The following functions have been removed:
# - res_temperature_min_rule
# - res_temperature_min_slack_rule
# - res_temperature_max_rule
# - res_temperature_fix_rule
# - def_initial_temperature_rule
# - def_startofperiod_temperature_rule
